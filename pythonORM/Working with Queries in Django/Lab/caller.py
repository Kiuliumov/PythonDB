...
def find_authors_nationalities():
    ...
    for author in all_authors:
        if author.nationality:
            all_authors_string += f'{author.first_name} {author.last_name} is {author.nationality}\n'
    return all_authors_string.strip()


def order_books_by_year():
    all_books = Book.objects.all().order_by('publication_year', 'title')
    all_books_string = ''
    for book in all_books:
        all_books_string += f'{book.publication_year} year: {book.title} by {book.author}\n'
    return all_books_string.strip()
...
def filter_authors_by_nationalities(nationality):
    ...
    for author in all_authors:
        ...
        if author.biography:
            all_authors_string += author.biography + '\n'
        else:
            all_authors_string += f'{author.first_name} {author.last_name}\n'
    return all_authors_string.strip()


def filter_authors_by_birth_year(first_year, second_year):
    all_authors = Author.objects.filter(birth_date__year__range=(first_year, second_year)).order_by('birth_date').reverse()
    all_authors_string = ''
    for author in all_authors:
        all_authors_string += f'{author.birth_date}: {author.first_name} {author.last_name}\n'
    return all_authors_string


def change_reviewer_name(old_name, new_name):
    all_reviews = Review.objects.filter(reviewer_name=old_name)
