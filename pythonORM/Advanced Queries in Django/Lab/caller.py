...
def product_quantity_ordered():
    products = Product.objects.annotate(total_quantity=Sum('orderproduct__quantity')).exclude(total_quantity=None).order_by('-total_quantity')
    return_str = ''
    for ordered_product in products:
        return_str += f'Quantity ordered of {ordered_product.name}: {ordered_product.total_quantity}\n'
...
def give_discount():
    ...
    return_str = ''
    for product in products:
        return_str += f'{product.name}: {product.price}lv.\n'
    return return_str.rstrip()
